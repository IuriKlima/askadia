...
export function CompanyJourney({company,area,actions,email,calendarDay}:{calendarDay?:string;company:{id:string;name:string};area:string;actions:string[];email:string}){
 ...
 return <div className={styles.shell}><a className={styles.skipLink} href="#company-content">Pular para o conteúdo</a>
  ...
  <aside id="company-navigation" className={styles.sidebar} data-open={mobile}><div className={styles.brandRow}><Link href="/" className="brand"><span className="brand-mark">a</span>askadia.</Link><button className={styles.mobileButton} aria-label="Fechar menu" onClick={()=>setMobile(false)}><X size={20}/></button></div><Link className={styles.context} href="/entrada"><Building2 size={18}/><span><small>EMPRESA ATUAL</small><strong>{data?.state.facts.name?.value??company.name}</strong><small>Trocar empresa ou contexto</small></span></Link><nav aria-label="Navegação da empresa" onClick={()=>setMobile(false)}>{nav.map(g=><div className={styles.navGroup} key={g.title}><small>{g.title}</small>{g.items.map(([key,label])=><Link key={key} href={href(key!)} aria-current={area===key?'page':undefined}>{label}</Link>)}</div>)}</nav><footer><small>{email}</small><Link href="/workspace">Gerenciar meus negócios</Link></footer></aside>
