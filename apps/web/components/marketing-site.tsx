...
export function MarketingShell({children,focused=false}:{children:ReactNode;focused?:boolean}) {
  return <div className={s.site}><a href="#conteudo" className={s.skip}>Pular para o conteúdo</a>
    <header className={s.header}><div className={s.headerInner}>
      <Link href="/" className={s.brand} aria-label="Askadia, início"><BrandWordmark/></Link>
...
export function ProductPreview({kind='marketing'}:{kind?:'marketing'|'inbox'}) {
  return <figure className={s.productFigure}><div className={s.productWindow}>
    <div className={s.windowBar}><div><i/><i/><i/></div><span>Seu espaço na Askadia</span><span className={s.windowMark}><BrandMark/></span></div>
    <div className={s.productBody}><aside className={s.previewNav} aria-hidden="true"><BrandMark/><Target size={18}/><CalendarDays size={18}/><MessageCircle size={18}/></aside><div className={s.previewMain}><div className={s.previewHeading}><span>ACADEMIA HORIZONTE</span><span>Seu negócio</span></div>
