...
export default async function CompanyPage({params}:{params:Promise<{id:string;section?:string[]}>}){
 ...
 if(error||!cap?.actions?.includes(permissions[area]))return <main className="internal-shell"><section className="panel internal-empty"><h1>Acesso indisponível.</h1><p>Seu perfil não permite abrir esta área ou o vínculo não está mais ativo.</p><Link className="button button-outline" href="/entrada">Trocar contexto</Link></section></main>;
 ...
 return <CompanyJourney key={id+':'+area} company={company} area={area} actions={cap.actions} email={auth.user.email??''}/>;
}
