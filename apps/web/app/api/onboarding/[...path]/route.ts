...
GET:[new RegExp('^companies/'+uuid+'/(overview|market)$'),new RegExp('^companies/'+uuid+'/site$'),new RegExp('^companies/'+uuid+'/ads(/(accounts|hierarchy))?$'),new RegExp('^companies/'+uuid+'/inbox/meta/(threads|messages)$'),new RegExp('^companies/'+uuid+'/crm/contacts/'+uuid+'/history$'),new RegExp('^companies/'+uuid+'/campaigns/recipients$'),new RegExp('^companies/'+uuid+'/management$'),new RegExp('^companies/'+uuid+'/campaigns$'),new RegExp('^companies/'+uuid+'/inbox(/(threads|messages|media))?$'),new RegExp('^companies/'+uuid+'(/(channels|calendar|videos/'+uuid+'|creatives/'+uuid+'|strategy|crm|attachments/'+uuid+'))?$'),new RegExp('^access/'+uuid+'$')],
POST:[new RegExp('^companies/'+uuid+'/market$'),new RegExp('^companies/'+uuid+'/site/(generate|save|publish|domain|subdomain|verify-domain)$'),new RegExp('^companies/'+uuid+'/ads/(google/start|select|disconnect|propose)$'),new RegExp('^companies/'+uuid+'/inbox/meta/(takeover|send)$'),new RegExp('^companies/'+uuid+'/campaigns/recipients/(register|revoke)$'),new RegExp('^companies/'+uuid+'/strategy/edit$'),new RegExp('^companies/'+uuid+'/management/key$'),new RegExp('^companies/'+uuid+'/campaigns/(save|preview|activation|students/import)$'),new RegExp('^companies/'+uuid+'/inbox/(settings|quick-replies|generate-prompt|crm-sync|release|takeover|suggest|preview|send|media-send)$'),/^companies$/,new RegExp('^companies/'+uuid+'/(channels/meta/start|channels/meta/pages|channels/meta/select|channels/evolution/connect|channels/evolution/status|channels/disconnect|calendar/prepare|calendar/generate|calendar/plan-dates|calendar/date|calendar/video/'+uuid+'/'+uuid+'|calendar/edit|calendar/approve|calendar/design|answers|places|website|strategy|strategy/generate|strategy/approve|crm/contacts|crm/stage|crm/conversations|crm/mode|crm/notes|attachments/'+uuid+')$'),new RegExp('^access/'+uuid+'/meetings$')],
};
async function forward(request:Request,context:{params:Promise<{path:string[]}>}){
  ...
  try{
    const response=await fetch((process.env.API_INTERNAL_URL||'http://127.0.0.1:4000')+'/onboarding/'+path+query,{
      ...
      body:body as BodyInit|undefined,cache:'no-store',signal:AbortSignal.timeout(isVideo?120000:path.endsWith('/calendar/design')?150000:path.endsWith('/calendar/generate')?110000:path.endsWith('/strategy/generate')||path.endsWith('/calendar/plan-dates')?60000:path.endsWith('/market')||path.includes('/ads/')||path.endsWith('/site/generate')?120000:path.endsWith('/overview')||path.includes('/channels/')||path.includes('/inbox/')||path.endsWith('/history')?60000:20000),
    });
