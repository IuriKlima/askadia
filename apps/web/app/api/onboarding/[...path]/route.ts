...
async function forward(request:Request,context:{params:Promise<{path:string[]}>}){
  ...
  try{
    const response=await fetch((process.env.API_INTERNAL_URL||'http://127.0.0.1:4000')+'/onboarding/'+path+query,{
      ...
      body:body as BodyInit|undefined,cache:'no-store',signal:AbortSignal.timeout(isVideo?120000:path.endsWith('/calendar/design')?150000:path.endsWith('/calendar/generate')?110000:path.endsWith('/strategy/generate')||path.endsWith('/calendar/plan-dates')?60000:path.includes('/ads/')||path.endsWith('/site/generate')?120000:path.includes('/channels/')||path.includes('/inbox/')||path.endsWith('/history')?60000:20000),
    });
