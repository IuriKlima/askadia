...
export class OnboardingController {
 ...
 @Get('companies/:id/crm') async crm(@Req() req:AuthRequest,@Param('id') id:string){await capability(req.actor,uuid(id),'crm.read');const [contacts,opportunities,conversations,notes]=await Promise.all([req.actor.client.from('contacts').select('*').eq('company_id',id).order('created_at',{ascending:false}).limit(200),req.actor.client.from('opportunities').select('*').eq('company_id',id).order('created_at',{ascending:false}).limit(200),req.actor.client.from('company_conversations').select('*').eq('company_id',id).order('updated_at',{ascending:false}).limit(200),req.actor.client.from('company_conversation_notes').select('*').eq('company_id',id).order('created_at',{ascending:false}).limit(200)]);return {contacts:result(contacts),opportunities:result(opportunities),conversations:result(conversations),notes:result(notes)};}
