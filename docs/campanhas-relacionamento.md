...
Testes locais exercitam isolamento, autorização, consentimento, atualização dos dados, versões, duplicação, mudança de presença, resultado incerto, chave revogada e repetição de importações. Não enviam mensagens reais. As três migrações foram aplicadas com autorização explícita no Supabase e a RLS foi conferida. Commit 6189426 implantado no Easypanel. A interface pública carregou as duas modalidades e a base vazia com o fuso real da empresa. O serviço de campanhas foi habilitado após autorização específica. Nenhum aluno foi importado, nenhuma campanha ativada e nenhuma mensagem enviada na homologação; recebimento por ERP e entrega real seguem dependentes de dados/ativação do proprietário.
