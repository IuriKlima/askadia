...
Publicar site também solicita o provisionamento do subdomínio reservado e do domínio próprio já verificado. A solicitação ocorre depois da aprovação/publicação versionada no banco; falhas externas não desfazem a versão pública e o botão permite nova tentativa. A resposta distingue publicação no link Askadia, configuração pendente e HTTPS solicitado (não implica certificado já emitido). Nenhuma chave administrativa é entregue ao cliente da academia.
